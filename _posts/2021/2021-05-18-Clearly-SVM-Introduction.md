...
$D=\dfrac{\|WX_i+b\vert}{\Vert W \Vert}$这个公式的学名叫做**几何间隔**，几何间隔表示的是点到超平面的**欧氏距离**（还记得上次讲线性回归时强调要记住这个名称吧？）。
...
为了看起来简洁，我们令目标函数 $\min\dfrac{||W||^{2}}{2}$为 $\min f(W)$，约束条件 $y_{i} WX_{i}+b)-1\geqslant 0$为$g_{i}(W)\geqslant 0$ 。
...
- 当可行解$W$落在 $g_{i}(W)>0$内时，此时i这个样本点是“马路”之外的点；
- 当可行解$W$落在 $g_{i}(W)=0$内时，此时i这个样本点正好落在马路边上，也就是我们的支持向量！
...
$$
L(W,b,\lambda ) = f(W)-\sum_{1}^{n} \lambda_{i} g_{i}(W)  =\frac{||W||^{2}}{2} - \sum_{1}^{n}  \lambda_{i}y_{i}(WX_{i}+b)  +\sum_{1}^{n}  \lambda_{i} \label{1}\tag{1}
$$
...
将(2)式 $W=\sum_{1}^{n}  \lambda_{i}y_{i}X_{i}$ 代入$\eqref{1}$式，得：
...
- 对新样本 $X_{*}$的预测： $\text{sgn}(W^{T}X_{*}+b)，W^{T}X_{*}+b$为正则 $y_{*}=1$，为负则 $y_{*}=-1$,为0则拒绝判断。
